...
class Play1Controller extends LuckyBaseController with GetTickerProviderStateMixin{
  ...
  _initYourList(){
    ...
    for (int i = 0; i < 12; i++) {
      final bool condition = ValueUtils.instance.getPlay2Point();
      if (condition && remainingNumbersInA.isNotEmpty) {
        final int numberFromA = remainingNumbersInA.first;
        yourList.add(YourBean(content: "$numberFromA", reward: ValueUtils.instance.getPlay2Reward(), win: true));
      } else {
        int randomNumber;
        do {
          randomNumber = random.nextInt(100);
        } while (setA.contains(randomNumber));
        yourList.add(YourBean(content: "$randomNumber", reward: ValueUtils.instance.getPlay2Reward(), win: false));
      }
